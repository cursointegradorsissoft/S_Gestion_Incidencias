<?php
	...
	header("Content-Disposition: attachment; filename=tipo_comida.xls");

	echo "<section class='tipo_cambio'>";
	$codigo = $_REQUEST['cod'];
	$query = "select * from ttico where ticocod='$codigo'";
	$consul=funciones::listadoReturn($c,$query);
	echo "<form method='post'>";
		echo "<table>";
		echo "<th colspan=2>Modificar Tipo de Comida</th>";
	...
	if($_POST)
	{
		...
		{
			ValModTiCo($codigo,$_POST['nombre']);
