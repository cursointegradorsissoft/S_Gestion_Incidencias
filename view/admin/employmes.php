<section class='content-general'>
	...
	<section class='superior'>
		...
		<section class='left' style="width:400px !important" >Empresa: EMPLEADO DEL MES</section>
		<section class='right'>
			...
			<section class='acceso' style="background-image: url('../themes/images/excel.png');">
				<input type='button' class="exp_mensual" name="boton" value='Excel' id='btn2'>
			</section>
					...
					<table cellpadding="0" cellspacing="0">
						...
						if($_POST)
						{
							if($_POST['boton'] == "Seleccionar")
							{
								$codigo = $_POST['codigo'];
								$mes = $_POST['mes'];
								$cadena = "select tecod,temanual,temmes,concat_ws(' ',pernom,perapp),percod from temes inner join tpers on temper=percod ";
								if($codigo != NULL && $mes == NULL)
								{
									$query = "$cadena where tecod=$codigo group by tecod";
								}
								else if($codigo == NULL && $mes != NULL)
								{
									$query = "$cadena where temmes='".$mes."' group by tecod";
								}
								else
								{
									
									$query = "$cadena group by tecod";
								...
								{	
									...
									for($x=0;$x<mysql_num_fields($val);$x++)
									{
										if($x==0)
										{ 
											echo "<td style='text-align:right; padding-left:1% !important;'><a href='#?cod=$reg[0]'>".$reg[$x]."</a></td>";
										}
										else if( $x == 4)
										{
				...
				<fieldset>
					<legend>Datos del Registro</legend>
						...
						<table cellspacing="0" cellpadding="0">
							...
							<tr>
								<td>A&ntilde;o</td>
								<td class='td'><input type="text" name="aniomod" class='text1' onkeypress="ValNumero(this)" onblur="ValNumeroAnio(this)" maxlength="4"></td>
							</tr>
							<tr>
								<td>Mes</td>
								<td class='td'><input type="text" name="mesmod" class='text2' onkeypress="ValNumero(this)" onblur="ValNumero2(this)" maxlength="2"></td>
							</tr>

							<tr>
								<td>Empleado</td>
								<td class='td'><input type="text" name="empmod" class='caja4 seteo_caja' id="busqueda9" readonly="" maxlength="5" onkeypress="ValNumero(this)">
								<input type="text" name="nommod" class='text3' class='caja2' readonly=""></td>
							</tr>
				...
				<fieldset>
					<legend>Datos del Registro</legend>
						...
						<table cellspacing="0" cellpadding="0">
							...
							<tr>
								<td>Empleado</td>
								<td class='td'><input type="text" name="codemp" class='caja4 seteo_caja' readonly="" id="busqueda10" maxlength="5" onkeypress="ValNumero(this)">
...
<?php
	if($_POST)
	{
		...
		{	
			...
			if($codmod != NULL)
			{
				$men=ValModMes($codmod,$_POST['aniomod'],$_POST['mesmod'],$_POST['empmod']);
			}
			else
			{
			    $men=ValRegMes($codnot,$_POST['anionew'],$_POST['mesnew'],$_POST['codemp']);
			}
