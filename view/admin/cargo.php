<section class='content-general'>
	...
	<form method="post" name="registrar">
		<section class='left'>Empresa: CARGO</section>
		<section class='right'>
					...
					<table cellpadding="0" cellspacing="0">
						...
						if($_POST)
						{
							if($_POST['boton'] == "Seleccionar")
							{
								...
								{
									$query = "select * from tfunc where FUNCOD=$codigo group by funcod";
								}
								else if($codigo == NULL && $area != NULL)
								{
									$query = "select * from tfunc where (FUNNOM LIKE '%' '".$area."' '%') group by funcod";
								}
								else
								{
...
<?php
	if($_POST)
	{
		...
		{	
			...
			if($codmod != NULL)
			{
				$men=valModCar($codmod,$_POST['nomcar']);
			}
			else
			{
			    $men=validarCargo($codnot,$_POST['nombre']);
			}
