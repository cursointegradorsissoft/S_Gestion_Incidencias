...
JSGantt.TaskItem = function(pID, pName, pStart, pEnd, pColor, pLink, pMile, pRes, pComp, pGroup, pParent, pOpen, pDepend, pCaption)
{
      ...
      this.getResource = function(){ if(vRes) return vRes + ' Dias'; else return '&nbsp';  };
      this.getCompVal  = function(){ if(vComp) return vComp; else return 0; };
      this.getCompStr  = function(){ if(vComp) return vComp+' Dias'; else return ' 0 Dias'; };
   ...
   this.Draw = function()
   {
      ...
      if(vTaskList.length > 0)
      {
         ...
         if ( document.getElementById("contenedor")){
            article = document.getElementById("contenedor");
         }else{
            article = document.getElementById("contenedor2");
         }
         var altDiv = article.clientHeight-5;
         var ancDiv1 = Math.round(article.clientWidth * 0.4,2)-2;
         var ancDiv2 = Math.round(article.clientWidth * 0.6,2)-2;
         ...
         vLeftTable =
            '<DIV class=scroll id=leftside style="width:' + ancDiv1 + 'px; height: ' + altDiv + 'px" onscroll=scroll1_div() >'+
         ...
         {  
            ...
            if(vTaskList[i].getVisible() == 0) 
               ...
               vLeftTable += '<TR id=child_' + vID + ' bgcolor=#' + vBGColor + ' onMouseover=g.mouseOver(this,' + vID + ',"left","' + vRowType + '") onMouseout=g.mouseOut(this,' + vID + ',"left","' + vRowType + '")  ondblclick=historial('+ vTaskList[i].getLink() +')>' ;
            ...
            vLeftTable += 
               '<spanstyle="cursor:pointer" nomrap> ' + vTaskList[i].getName() + '</span></NOBR>'+elemento+'</TD>' ;
         ...
         vRightTable = 
         '<TD style="width: ' + ancDiv2 + 'px; height: ' + altDiv + 'px" vAlign=top bgColor=#ffffff>' +
         '<DIV class=scroll2 id=rightside onscroll=scroll2_div() style="width: ' + ancDiv2 + 'px; height: ' + altDiv + 'px">' +
         '<TABLE style="width: ' + ancDiv2 + 'px;" cellSpacing=0 cellPadding=0 border=0>' +
         ...
         {  
            if(vFormat == 'day' )
            {
               if( JSGantt.formatDateStr(vCurrDate,'mm/dd/yyyy') == JSGantt.formatDateStr(vTmpDate,'mm/dd/yyyy')) {
                  vWeekdayColor  = "ccccff"; vWeekendColor  = "9999ff";
               } else {
                  vWeekdayColor = "ffffff";  vWeekendColor = "cfcfcf";
               }
               ...
               for(u = 0; u < vTaskList.length; u++){
                  var bck;
                  if ( Date.parse(vTmpDate)>= Date.parse(vTaskList[u].getStart()) )
                  {
                     bck="background:red !important;";
                  }
                  //alert(Date.parse(vTmpDate) + " --- " + Date.parse(vTaskList[u].getStart())) ;
                  ...
                  if(vTmpDate.getDay() % 6 == 0) {
                     ...
                     vItemRowStr  += '<td class="gheadwkend" id="gheadwkend" style="'+css2+' HEIGHT: 19.9px; cursor: default;"  bgcolor=#' + vWeekendColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp</div></td>';
                  } else {
            ...
            if( vTaskList[i].getGroup()) {
               ...
               '<div id=bardiv_' + vID + ' style="position:absolute; top:5px; left:' + Math.ceil(vTaskLeft * (vDayWidth) + 1) + 'px; height: 7px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px">' +
                 '<div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" class=gtask style="background-color:rgba(0,51,102,1); height: 7px; width:' + Math.ceil((vTaskRight) * (vDayWidth) -1) + 'px;  cursor: pointer;opacity:0.9;">' +
                   '<div style="Z-INDEX: -4; float:left; background-color:rgba(0,51,102,1); height:3px; overflow: hidden; margin-top:1px; ' +
                         ...
                         'cursor: pointer;" onclick=historial("' + vTaskList[i].getLink() + '");>' +
                     '</div>' +
                  '</div>' +
               ...
               vDivStr = '<DIV>'+
                           '<TABLE style="position:relative; top:0px; width: ' + vChartWidth + 'px;" cellSpacing=0 cellPadding=0 border=0>' +
                              '<TR id=childrow_' + vID + ' class=yesdisplay style="HEIGHT: 20px" bgColor=#ffffff onMouseover=g.mouseOver(this,' + vID + ',"right","row") onMouseout=g.mouseOut(this,' + vID + ',"right","row")>' + vItemRowStr + 
                              '</TR>'+
                           '</TABLE>'+
                        '</DIV>';
               ...
               var fec_ini = vTaskList[i].getStart();
               var g1 = "#FAAC58";
               var g2 = "#FF4000";

               var dias_transcu = Math.ceil((fec_act - fec_ini) / (24 * 60 * 60 * 1000));
               var dias_totales = Math.ceil((vTaskList[i].getEnd() - vTaskList[i].getStart()) /  (24 * 60 * 60 * 1000) + 1);
               var porcentaje_t = Math.round((100/dias_totales)*dias_transcu)+"%";
               if( dias_totales< dias_transcu){porcentaje_t=100+"%";}

               if ( Math.ceil(Date.parse(fec_act)) > Math.ceil(Date.parse(fec_ini) ) ){
                  g1="#58ACFA"; g2="#045FB4";
               }
               ...
               vRightTable +=
                  '<div id=bardiv_' + vID + ' style="position:absolute; top:4px; left:' + size_left + 'px; height:18px; width:' + size_right + 'px">' +
                     '<div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" class=gtask style="background:'+g1+'; height: 13px; width:' + size_right + 'px; cursor: pointer;opacity:0.9;" ' +
                        'onclick=historial("' + vTaskList[i].getLink() + '"); >' +
                        '<div class=gcomplete style="Z-INDEX: -4; float:left; background-color:'+g2+'; height:5px; overflow: auto; margin-top:4px; filter: alpha(opacity=40); opacity:0.4; width:' + porcentaje_t + '; overflow:hidden">' +
                        '</div>' +
                     '</div>';
...
function fecha_actual(){
   ...
   var fdia ,fmes;
   mess<10?fmes="0"+mess:fmes=mess;
   dias<10?fdia="0"+dias:fdia=dias;
   var fec_act = fdia+"/"+fmes+"/"+anio;
   return fec_act;
}
